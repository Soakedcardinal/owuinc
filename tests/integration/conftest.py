            ...
            try:
                with urllib.request.urlopen(req, timeout=1) as resp:
                    ...
                    if status in (200, 207):
                        ...
                        logger.info(
                            f">>> Server ready at {url} (HTTP {status}, attempt {attempt}, {elapsed:.1f}s)"
                        )
                        return
                    logger.debug(
                        f">>> Poll attempt {attempt}: HTTP {status} (waiting for 200/207/401)"
                    )
            except urllib.error.HTTPError as http_err:
                if http_err.code == 401:
                    elapsed = time.time() - start
                    logger.info(
                        f">>> Server ready at {url} (HTTP 401, attempt {attempt}, {elapsed:.1f}s)"
                    )
                    return
                logger.debug(
                    f">>> Poll attempt {attempt}: HTTP {http_err.code} (waiting for 200/207/401)"
                )
...
"""
    ...
    if radicale_exe is None:
        ...
        if not radicale_exe.exists():
            raise RuntimeError(
                "Radicale not found on PATH or in .venv/bin — install radicale or ensure it's on PATH"
            )
    ...
    logger.info(
        f">>> Starting Radicale on port {RADICALE_PORT} (cmd: {radicale_exe} --config {config_file})"
    )
...
async def caldav_tools(radicale_server):
    ...
    Note: Overrides _caldav_client to use async Radicale client with
    ...
    logger.info(
        f">>> Tools valves configured: base_url={t.valves.NEXTCLOUD_BASE_URL}, username={t.valves.NEXTCLOUD_USERNAME}, sandbox={t.valves.SANDBOX_DIR}"
    )

    original_caldav_client = Tools._caldav_client

    async def rad_caldav_client(self):
    ...
    Tools._caldav_client = rad_caldav_client

    try:
        logger.info(">>> Yielding caldav_tools instance")
        yield t
    finally:
        Tools._caldav_client = original_caldav_client
        logger.info(">>> Restored original Tools._caldav_client method")
...
""")
    ...
    logger.info(
        f">>> Starting WsgiDAV on port {WSGIDAV_PORT} (cmd: {python_exe} {script_path})"
    )
    proc = subprocess.Popen(
...
async def webdav_tools(wsgidav_server):
    ...
    logger.info(
        f">>> Tools valves configured: base_url={t.valves.NEXTCLOUD_BASE_URL}, webdav_user={t.valves.WEBDAV_USERNAME}, sandbox={t.valves.SANDBOX_DIR}"
    )
