...
Password input only masks values in the UI, it does not change how they are stored. By default, Valve values (including secrets) are saved as plaintext JSON in the database. To encrypt them at rest, an administrator can set ENABLE_VALVE_ENCRYPTION=true, which encrypts Tool and Function Valve values with Fernet (AES-128) using a key derived from WEBUI_SECRET_KEY.
warning

When encryption is enabled, WEBUI_SECRET_KEY must be a valid 44-character url-safe base64 Fernet key, and rotating it makes previously encrypted Valve values unreadable (they must be re-entered). See ENABLE_VALVE_ENCRYPTION for details.
